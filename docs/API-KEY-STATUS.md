...
**Deployed:** 2026-07-21 — this `.env` is now live on the homelab (`ubuntu-g2`, commit
`eede3af`). The 10 working keys are active in production. AISStream (item 3) still
needs the reachability re-check *on the server* — see below.
...
itself, not a header/format issue — the endpoint _is_ gated (without any token it
...
here. AISStream authenticates via the subscribe message _after_ connecting, so the
