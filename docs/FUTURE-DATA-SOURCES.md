...
| **CelesTrak** | Satellites (orbital) | `celestrak.org` (TLE/JSON) | none | Open, attribution | Good | Keyless satellite tracking. Can serve as the keyless fallback for the space layer that currently needs `N2YO_API_KEY` | Medium |
...
| **Feodo Tracker** | Botnet C2 IPs | `feodotracker.abuse.ch` | none | CC0 | Good | Live botnet C2 IP list — geolocate onto the map as a cyber-threat layer | Low–Medium |
...
| **URLhaus** | Malicious URLs | `urlhaus-api.abuse.ch` | none | CC0 | Good | Lookup/feed of malicious URLs + hosting. Strengthens the Cyber tab | Low–Medium |
| **ThreatFox** | IOCs (IP/domain/hash) | `threatfox-api.abuse.ch` | none | CC0 | Good | IOC search — pivots an IP/domain/hash to known-malicious context. Natural fit next to VirusTotal/AbuseIPDB | Low–Medium |
...
`configured` gated on feed presence). ADS-B / AIS / APRS local collection is a *robustness*
...
Imagery is *raster* — it needs a different render path (MapLibre raster/tile source or
...
new *visual* dimension (change detection, SAR through cloud/night, nighttime-lights economics).
...
Worth weighing against the source list — a few of these make the data you *already* have
