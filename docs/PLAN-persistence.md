...
- **Phase 5 — timeline scrubber — DONE (2026-07-21).** Append-on-CHANGE `entity_observations` table (severity/lat/lon over time), written inside `reconcile()` only when an entity is new or its state changed (so an unchanging entity is one baseline row, not one per reconcile). `getSnapshotAt(layer, at)` reconstructs a layer at any past instant: latest observation ≤ `at`, filtered to entities whose event lifespan actually covered `at`. Read route `GET /api/snapshot/:layer?at=ISO` (same graceful-off + ISO-validation as `/api/changes`). Retention prunes observations on the same window as events/alerts. Frontend: a slider in the What-Changed tab (`scrubberTime`/`replayEntities` pure + tested) that replays the map (`renderMap` replay branch, reusing `buildIconLayer`) + a click-to-fly list; far-right = live. Backend live-verified (43 seismic observations reconstructed); DB-gated DOM path wants a homelab Playwright pass. **Known limitation:** an entity whose last state change predates the retention window loses its baseline, so replay near the window edge can under-represent a rare long-lived, never-changing entity — acceptable for these event-shaped layers.
